...
def non_max_suppression(inputs, model_size, max_output_size,
    ...
    '''Non-Maximum Supression (NMS) eliminates all proposed bboxes for 
    the same object except the bbox with the highest confidence score.'''
...
def resize_image(inputs, modelsize):
    '''Takes a tensor array and resize it for inference.

    The modelsize is as tuple with the desired width and height.'''
...
def load_class_names(file_name):
    ...
    The source is a file with class names provided by yolo.'''
...
def output_boxes(inputs, model_size, max_output_size, max_output_size_per_class,
    ...
    '''Returns predictions in the form of a tuple of tensors.
    
    The tuple contains boxes, scores, classes and number of detections.'''
...
def draw_outputs(img, boxes, objectness, classes, nums, class_names):
    '''Takes the original 3D uint8 numpy array plus inference outputs 
    to draw on images.'''
